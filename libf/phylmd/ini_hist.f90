...
contains
  ...
  subroutine ini_histhf(dtime, presnivs, nid_hf, nid_hf3d)
    ...
    CALL histbeg_totreg("histhf", iim, zx_lon(:, 1), (jjm + 1), &
  ...
  subroutine ini_histhf3d(dtime, presnivs, nid_hf3d)
    ...
    CALL gr_fi_ecrit(1, klon, iim, (jjm + 1), rlat, zx_lat)
    CALL histbeg_totreg("histhf3d", iim, zx_lon(:, 1), (jjm + 1), &
         zx_lat(1, :), 1, iim, 1, (jjm + 1), itau_phy, zjulian, dtime,  &
         nhori, nid_hf3d)
  ...
  subroutine ini_histins(dtime, presnivs, ok_instan, nid_ins)
    ...
    IF (ok_instan) THEN
       ...
       ENDDO
       CALL gr_fi_ecrit(1, klon, iim, (jjm + 1), rlat, zx_lat)
       CALL histbeg_totreg("histins", iim, zx_lon(:, 1), (jjm + 1), &
            zx_lat(1, :), 1, iim, 1, (jjm + 1), itau_phy, zjulian, dtime, &
            nhori, nid_ins)
       ...
       CALL histdef(nid_ins, "solldown", "Down. IR rad. at surface",  &
            "W/m2", iim, (jjm + 1), nhori, 1, 1, 1, -99, 32,  &
            "inst(X)", zsto, zout)
