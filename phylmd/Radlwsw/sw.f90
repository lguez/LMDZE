...
contains
  ...
  SUBROUTINE SW(PSCT, PRMU0, PFRAC, PPMB, PDP, PPSOL, PALBD, PALBP, PTAVE, &
    ...
    DOUBLE PRECISION PDP(KDLON, LLM) ! LAYER THICKNESS (PA)
    ...
    DOUBLE PRECISION PQS(KDLON, LLM) ! SATURATED WATER VAPOUR (KG/KG)
    DOUBLE PRECISION POZON(KDLON, LLM) ! OZONE CONCENTRATION (KG/KG)
    DOUBLE PRECISION PCLDSW(KDLON, LLM) ! CLOUD FRACTION
    DOUBLE PRECISION PTAU(KDLON, 2, LLM) ! CLOUD OPTICAL THICKNESS
    DOUBLE PRECISION POMEGA(KDLON, 2, LLM) ! SINGLE SCATTERING ALBEDO
    DOUBLE PRECISION PCG(KDLON, 2, LLM) ! ASYMETRY FACTOR
    DOUBLE PRECISION PHEAT(KDLON, LLM) ! SHORTWAVE HEATING (K/DAY)
    DOUBLE PRECISION PHEAT0(KDLON, LLM)! SHORTWAVE HEATING (K/DAY) clear-sky
    ...
    DOUBLE PRECISION ZFSUP(KDLON, LLM+1)
    DOUBLE PRECISION ZFSDN(KDLON, LLM+1)
    DOUBLE PRECISION ZFSUP0(KDLON, LLM+1)
    DOUBLE PRECISION ZFSDN0(KDLON, LLM+1)
    ...
    DOUBLE PRECISION ZCLD(KDLON, LLM)
    DOUBLE PRECISION ZCLEAR(KDLON)
    DOUBLE PRECISION ZDSIG(KDLON, LLM)
    DOUBLE PRECISION ZFACT(KDLON)
    DOUBLE PRECISION ZFD(KDLON, LLM+1)
    DOUBLE PRECISION ZFDOWN(KDLON, LLM+1)
    DOUBLE PRECISION ZFU(KDLON, LLM+1)
    DOUBLE PRECISION ZFUP(KDLON, LLM+1)
    DOUBLE PRECISION ZRMU(KDLON)
    DOUBLE PRECISION ZSEC(KDLON)
    DOUBLE PRECISION ZUD(KDLON, 5, LLM+1)
    DOUBLE PRECISION ZCLDSW0(KDLON, LLM)
    ...
    DOUBLE PRECISION, save, allocatable:: ZFSUPAD(:, :) ! (KDLON, LLM+1)
    DOUBLE PRECISION, save, allocatable:: ZFSDNAD(:, :) ! (KDLON, LLM+1)
    ...
    if(.not.initialized) then
       initialized=.TRUE.
       allocate(ZFSUPAD(KDLON, LLM+1), ZFSDNAD(KDLON, LLM+1))
       ZFSUPAD = 0.
       ZFSDNAD = 0.
    endif
    !rv
