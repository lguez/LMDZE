    ...
    DO jk = klev, ilevh, - 1
       DO jl = 1, klon
          IF (jk>=kknub(jl) .AND. jk<=kknul(jl)) THEN
             pulow(jl) = pulow(jl) + pum1(jl, jk) * (paphm1(jl, jk + 1) &
                  - paphm1(jl, jk))
             pvlow(jl) = pvlow(jl) + pvm1(jl, jk) * (paphm1(jl, jk + 1) &
                  - paphm1(jl, jk))
          END IF
       end DO
    end DO
    DO jl = 1, klon
       pulow(jl) = pulow(jl) / (paphm1(jl, kknul(jl) + 1) &
            - paphm1(jl, kknub(jl)))
       pvlow(jl) = pvlow(jl) / (paphm1(jl, kknul(jl) + 1) &
            - paphm1(jl, kknub(jl)))
       ...
       DO jl = 1, klon
          IF (ktest(jl) == 1) THEN
             IF (jk>=(kknub(jl) + 1) .AND. jk<=kknul(jl)) THEN
                ...
                prho(jl, klev + 1) = prho(jl, klev + 1) + paphm1(jl, jk) * 2. &
                     * zdp(jl, jk) * zcons1 / (ptm1(jl, jk) + ptm1(jl, jk - 1))
             END IF
          END IF
       end DO
    end DO
       ...
       DO jl = 1, klon
          ...
          IF (ktest(jl) == 1) THEN
             ...
             IF (jk>=kknub(jl)) THEN
                ...
                zrhom = prho(jl, jk)
                zrhop = prho(jl, jk + 1)
                pnu(jl) = pnu(jl) + (zdelp / rg) * ((zstabp / zrhop + zstabm &
                     / zrhom) / 2.) / zwind
    ...
    DO jk = klev - 1, 2, - 1
       DO jl = 1, klon
          ...
          IF (ktest(jl) == 1) THEN
             ...
             zrhom = prho(jl, jk)
             zrhop = prho(jl, jk + 1)
             znup(jl) = znup(jl) + (zdelp / rg) * ((zstabp / zrhop + zstabm &
                  / zrhom) / 2.) / zwind
